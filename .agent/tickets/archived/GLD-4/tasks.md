...
- [x] Update `README.md` — setup, env vars, dev commands, architecture overview
- [x] Update `.agent/techContext.md` — stack is **Next.js 16** (not Electron) per GLD-4 decisions
- [x] Verify acceptance: live rate on open, 30s auto-refresh, 8g pawn hero, karat toggle, last-updated + error states
- [x] Run `npm run lint` and `npm run build` — fix any failures
- [x] Run `npm test` (or configured test script) — all gold-rate unit tests pass
